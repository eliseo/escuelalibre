...
function getClases(){
	global $ndb;
	$info=array();
	$query = "	SELECT DISTINCT c.id, c.name_course, c.desc_course, c.start_date, c.end_date, r.desc AS room 
				FROM courses c 
...
function getStudientsByCourse($hid=false, $id_course=false){
	global $ndb;
	$info=array();
	$query = "	SELECT stu.hid, stu.f_lastname, stu.s_lastname, stu.name, stu.email 
				FROM courses c 
	...
	foreach($data as $col){
		...
		$ratings[$hid][] = array(
							...
							"rating" => $col->rating,
							"comment" => htmlentities($col->comment)
						);
	}
...
function getSchedule($hid=false, $id_course = false ){
	...
	if($usertype->func_1 == 1){
		...
		$query = "SELECT 	a.horario, 
					...
					FROM courses c 
					JOIN daily_schedule ds ON c.id = ds.id_course "
					.($id_course ? " WHERE ds.id_course = $id_course " : "").
					" GROUP BY CONCAT(start_time, ' - ', end_time)
					) a, 
					(
					SELECT 	
...
function getAssistance($hid=false, $id_course=false){
	...
	$info=array();
	$query = "	SELECT 
					DISTINCT p.lun, p.mar, p.mier, p.juev, p.vier, p.sab, p.dom, c.start_date, c.end_date 
				FROM courses c 
