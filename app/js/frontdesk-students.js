	...
	if($.isArray(data)){
		...
		$.each(data, function(i, item){
			...
			var info = '<tr>';
				...
				info += '<td class="odd gradeA"><a href="students?id_student='+item.id_user +'">' + item.user + '</a></td>';
...
function getStudentsList(){
	$.getJSON("datastores/frontdeskdata.php?q=studentslist&left=1", showStudentsList);
}


function getStudentInfo(id_student,nodeI,nodeC){
	$.getJSON("datastores/frontdeskdata.php?q=studentdata&id_student="+id_student,function(data){
