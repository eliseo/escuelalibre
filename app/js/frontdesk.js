...
function showSelectProfesor(data){
	if($.isArray(data)){
		...
		$.each(data, function(i, item){
			optionP =optionP+"<option value='"+item.id_profesor+"'>"+item.f_lastname+","+item.name_profesor+"</option>";
		});
...
function infoCourse(id_course,nodeG,nodeDs){
	$.getJSON("datastores/admindata.php?q=coursedata&id_course="+id_course,function(data){
		...
		$.each(data, function(i, item){
				...
				info += '<tr>';
				info +='<th>Name Course:</th>';
				info +='<td>'+item.name_course+'</td>';
				info += '</tr>';
				
				info += '<tr>';
				info +='<th>Desc Course:</th>';
	...
	if($.isArray(data)){
		...
		$.each(data, function(i, item){
			...
			var info = '<tr class="gradeA odd">';
				info += '<td><a id="course_'+item.id_course+'" href="courses?id_course='+item.id_course +'">' + item.id_course + '</a></td>';
				info += '<td>' + item.name_course + '</td>';
