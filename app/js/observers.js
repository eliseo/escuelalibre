...
function cargarListaDocumentos(id_course){
	$.post('datastores/observerdata.php?q=listamateriales&id_course='+id_course, function(response){
...
function verDetalle(ref){
	// Se cancela el comportamiento predeterminado del hipervínculo
		...
		$.post('datastores/observerdata.php?q=horarios&id_course=' + id_course, function(response){
			//alert(response);
		...
		/// --------- Obtiene los alumnos
		$.post('datastores/observerdata.php?q=alumnos&id_course=' + id_course , function(response){
		...
		$.post('datastores/observerdata.php?q=asistencia&id_course=' + id_course , function(ds){
...
function cargarAsistencias(id_course, nav){
			...
			if(typeof fs.length == 'undefined'){
				//Si no hay datos disponbles solo se manda un mensaje de aviso
				$('#assistancetable').html('No hay datos disponibles');
				return;
			}
...
function cargarClases(){
	$.post('datastores/observerdata.php?q=cursos', function(response){
	...
	for(var sidx in students){
		...
		for( var eidx in evals ){
			if(evals[eidx].rating != null){
				datos += 'calif%5B' + evals[eidx].hid + '-' + evals[eidx].id_eval_type + '%5D' + '=' + evals[eidx].rating + '&';
			}
		}
	}
...
function cargarHorarioGeneral(){
	$.post('datastores/observerdata.php?q=horarios', function(response){
